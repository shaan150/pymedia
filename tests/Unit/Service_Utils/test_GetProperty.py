...
class TestGetProperty(unittest.TestCase):
    ...
    def test_missing_property_exception(self, mock_file, mock_json_load):
        with self.assertRaises(MissingPropertyException) as cm:
            get_property("missing_property", "valid_path.json")
        self.assertEqual(str(cm.exception), "missing_property not found in valid_path.json")
